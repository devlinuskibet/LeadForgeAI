...
class CompanyAnalysis(Base):
    ...
    company_id = Column(UUIDType, ForeignKey("companies.id"), nullable=False, index=True, unique=True)

    raw_scraped_text = Column(String, nullable=True)
    inferred_problems = Column(JSONType, nullable=True)  # [{"problem": "No mobile app", "severity": "High"}]
    recommended_solutions = Column(JSONType, nullable=True)  # [{"solution": "Build React Native App", ...}]
